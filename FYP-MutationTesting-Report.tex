...
After adding all six new test cases developed during Task 3, the full test suite was re-run to confirm every test passed:
...
\midrule
Mutation Score      & 42.41\%                  & 47.86\%                 & +5.45 pp        \\
Killed Mutants      & 109                      & 123                     & +14             \\
Survived Mutants    & 115                      & 101                     & $-$14           \\
New Tests Added     & ---                      & 6                       & ---             \\
\bottomrule
\end{tabular}
...
The six new tests killed 14 additional mutants, lifting the mutation score from 42.41\% to 47.86\%, an improvement of 5.45 percentage points. Two things stand out on reflection.

First, four of the fourteen killed mutants were \texttt{BooleanLiteral} mutations in response objects --- the tests that assert \texttt{success: false} on error responses and \texttt{success: true} on successful ones. This was the expected outcome since those tests directly targeted that class of mutation. However, they also reveal a broader pattern that the baseline tests had adopted: asserting only HTTP status codes and ignoring the body of the response. This is a weak testing habit. HTTP status codes are conventions; business logic lives in the response payload. Future tests should assert specific fields in every response, not just the status.

Second, the improvement of 5.45 points, while meeting the threshold, also shows how resistant the remaining survived mutants are. The dominant survivors are \texttt{ObjectLiteral} mutations that replace entire response objects with empty ones, and \texttt{BooleanLiteral} mutations that flip internal boolean flags. Killing these systematically would require asserting the complete structure of every response body in every test, which is achievable but time-consuming. The more revealing insight is that a substantial portion of survived mutants target the Socket.io emission logic (lines 86--90 and 186--190), which cannot be killed by unit tests that mock \texttt{io} as null. Those mutations require integration tests with a live server. Writing this report has made clear that the testing strategy for OrderIQ should include at least one integration test layer for real-time features.

Going forward, the most useful change to testing practice is to treat every assertion as an opportunity to check a boundary. The four mutant analyses in this report each uncovered a test that was checking the right thing in the wrong place: confirming an HTTP status while ignoring the exact condition that produced it. Boundary-value tests, specifically inputs designed around the exact edge of each conditional, are what kill mutations. That principle will shape how tests are written for the remainder of the project.
